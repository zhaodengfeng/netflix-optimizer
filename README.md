...
- Shows all subtitle tracks
- HEVC 4K HDR + Dolby Digital Plus/Atmos auto-enabled on Windows 11 + Edge
- Single on/off switch — no configuration needed
...
Click the extension icon to find a single **Enable Netflix Optimizer** switch (on by default).

- **On** — highest bitrate, 5.1 audio and all subtitle tracks; all codecs available. On Windows 11 + Edge, HEVC 4K HDR and Dolby Digital Plus/Atmos profiles are enabled automatically.
- **Off** — playback is left untouched (stock Netflix behavior).

> Changes apply after reloading the Netflix tab.
